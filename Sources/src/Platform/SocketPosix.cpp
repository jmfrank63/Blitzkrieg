...
bool GetLocalAddress(SocketHandle socketHandle, SocketAddress *address) { if (!address) return false; sockaddr_in native{}; socklen_t length = sizeof(native); if (getsockname(Native(socketHandle), reinterpret_cast<sockaddr*>(&native), &length) != 0) return false; FromNative(native, address); return true; }
...
bool SetBroadcast(SocketHandle socketHandle, bool enabled) { const int value = enabled ? 1 : 0; return setsockopt(Native(socketHandle), SOL_SOCKET, SO_BROADCAST, &value, sizeof(value)) == 0; }
