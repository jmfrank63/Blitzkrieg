...
void CImageView::OnPaint() 
{
	...
	CWnd::OnPaint();
	g_frameManager.GetActiveFrame()->GFXDraw();
}
...
void CImageView::UpdateScrolls( UINT nSBCode, UINT nPos, CScrollBar* pScrollBar )
{
  SCROLLINFO info;
  if ( !pScrollBar->GetScrollInfo( &info ) )
	...
	CImageFrame *pFrame = static_cast<CImageFrame *> ( g_frameManager.GetActiveFrame() );
