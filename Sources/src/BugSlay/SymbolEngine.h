...
Note: I had a whopper of a bug here.  I had the declarations and
    ...
    typedef BOOL (*PFNSYMGETLINEFROMADDR) ( IN  HANDLE         hProcess,
																						IN  DWORD          dwAddr,
																						OUT PDWORD         pdwDisplacement,
																						OUT PIMAGEHLP_LINE Line );
...
typedef BOOL (STDCALL *PFNSYMGETLINEFROMADDR)( IN  HANDLE hProcess,
																								 IN  DWORD dwAddr,
																								 OUT PDWORD pdwDisplacement,
																								 OUT PIMAGEHLP_LINE Line );

typedef BOOL (STDCALL *PFNSYMGETLINEFROMNAME)( IN  HANDLE hProcess,
																								 IN  LPSTR ModuleName,
																								 IN  LPSTR FileName,
																								 IN  DWORD dwLineNumber,
																								 OUT PLONG plDisplacement,
...
typedef BOOL (STDCALL *PFNSYMGETLINENEXT)( IN HANDLE hProcess,
                                             IN OUT PIMAGEHLP_LINE Line );

typedef BOOL (STDCALL *PFNSYMGETLINEPREV)( IN HANDLE hProcess ,
                                             IN OUT PIMAGEHLP_LINE Line );

typedef BOOL (STDCALL *PFNSYMMATCHFILENAME)( IN  LPSTR FileName,
                                               IN  LPSTR Match,
                                               OUT LPSTR *FileNameStop ,
                                               OUT LPSTR *MatchStop );
...
public:
  ...
  CSymbolEngine()
  {
    ...
    {
      m_pfnSymGetLineFromName = 0;
      m_pfnSymGetLineNext     = 0;
      m_pfnSymGetLinePrev     = 0;
      m_pfnSymMatchFileName   = 0;
    }
    else
    {
...
public:
  ...
  {
    // First try MSDBI.DLL.
    if ( TRUE == GetInMemoryFileVersion("MSDBI.DLL", dwMS, dwLS) )
      return TRUE;
