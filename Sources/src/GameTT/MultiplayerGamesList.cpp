...
{ 
	...
	case IMC_OK:
		if ( pDialogEnterPassword )
		{
			...
			const std::string szPassword = NStr::ToAscii( std::wstring( NPlatform::WideFromWordString( pPasswordText->GetWindowText( 0 ) ) ) );
...
void CInterfaceMPGamesList::UpdateServerInfo( SUIServerInfo * pServerInfo )
{
	...
	pPasswordDialog->SetWindowText( 0, NPlatform::WordStringData( NPlatform::WordStringFromWide( NStr::ToUnicode( NStr::Format( "%i", nState ) ).c_str() ) ) );
	...
	pElementServerName->SetWindowText( 0, NPlatform::WordStringData( NPlatform::WordStringFromWide( pServerInfo->szName.c_str() ) ) );

	IUIStatic * pElementModName = checked_cast<IUIStatic*>( pRow->GetElement( 2 ) );
	pElementModName->SetWindowText( 0, NPlatform::WordStringData( NPlatform::WordStringFromWide( NStr::ToUnicode( pServerInfo->szModName ).c_str() ) ) );
	
	IUIStatic * pElementModVersion = checked_cast<IUIStatic*>( pRow->GetElement( 3 ) );
	pElementModVersion->SetWindowText( 0, NPlatform::WordStringData( NPlatform::WordStringFromWide( NStr::ToUnicode( pServerInfo->szModVersion ).c_str() )  ) );

	IUIStatic *pMapName = checked_cast<IUIStatic*>( pRow->GetElement( 4 ) );
	pMapName->SetWindowText( 0, NPlatform::WordStringData( NPlatform::WordStringFromWide( SUIMapInfo::GetVisualName( pServerInfo->szMapName ) ) ) );
	...
	pPlayersNum->SetWindowText( 0, NPlatform::WordStringData( NPlatform::WordStringFromWide( NStr::ToUnicode( NStr::Format( "%d/%d", pServerInfo->nPlayers, pServerInfo->nPlayersMax ) ).c_str() ) ) );

	IUIStatic * pPing = checked_cast<IUIStatic*>( pRow->GetElement( 7 ) );
	pPing->SetWindowText( 0, NPlatform::WordStringData( NPlatform::WordStringFromWide( NStr::ToUnicode( NStr::Format( "%d", int(pServerInfo->fPing * 1000) ) ).c_str() ) ) );
