...
const gpu_debug = @import("builtin").mode == .Debug;
...
const vert_shader_source = @embedFile("shaders/rawTriangle.vert.spv");
const vert_shader_name = "Raw Triangle";
const frag_shader_source = @embedFile("shaders/solidColor.frag.spv");
...
fn loadGraphicsShader(
    ...
    const spirv_metadata = try sdl3.shadercross.reflectGraphicsSpirv(shader_code);
    return try sdl3.shadercross.compileGraphicsShaderFromSpirv(device, .{
        .bytecode = shader_code,
        .enable_debug = gpu_debug,
        .entry_point = "main",
        .name = name,
        .shader_stage = stage,
    }, spirv_metadata);
...
pub fn init(
    ...
    const device = try sdl3.gpu.Device.init(shader_formats, gpu_debug, null);
