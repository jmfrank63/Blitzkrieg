...
fn parseOptions(args: []const []const u8) !Options {
    ...
    while (i < args.len) {
        ...
        else if (std.mem.eql(u8, key, "--max-rgb")) result.max_rgb = try std.fmt.parseInt(u8, nextArg(args, &i), 10)
...
pub fn main(init: std.process.Init) !void {
    ...
    const passed = alpha_mismatches == 0 and max_error <= options.max_rgb and mean_error <= options.mean_rgb;
    var output_buffer: [512]u8 = undefined;
    var output = std.Io.File.stdout().writer(init.io, &output_buffer);
    try output.interface.print("max_rgb={d}/255 mean_rgb={d:.6}/255 alpha_mismatches={d} changed_pixels={d} pass={s}\n", .{ max_error, mean_error, alpha_mismatches, changed_pixels, if (passed) "true" else "false" });
    if (options.report) |report_path| {
        ...
        try report_writer.interface.print("max_rgb={d}/255\nmean_rgb={d:.6}/255\nalpha_mismatches={d}\nchanged_pixels={d}\npass={s}\n", .{ max_error, mean_error, alpha_mismatches, changed_pixels, if (passed) "true" else "false" });
    }
