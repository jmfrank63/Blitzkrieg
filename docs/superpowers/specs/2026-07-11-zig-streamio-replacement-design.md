...
The C++ ABI remains entirely synchronous. `IDataStream`, `IDataStorage`, `ISaveLoadSystem`, and option-system calls retain their existing blocking return, error, ordering, and lifetime semantics. The C++ bridge never exposes Zig operations, futures, batches, or completion callbacks, and it never invokes C++ from a worker thread. This makes the replacement a drop-in module for the 2003 engine while allowing new Zig-owned callers to overlap I/O safely.

The Zig-native API provides paired calls for every operation that can safely overlap: a blocking `read`/`write`/`decompress` call, plus `submitRead`/`submitWrite`/`submitDecompress` returning a request handle. The handle supports `await`, timed `await`, and `cancel`; completion is obtained by polling the owning completion queue or awaiting the handle. `IDataStream::Read` and equivalent C++ methods implement their synchronous behavior by submitting the same request and awaiting it, so there is one implementation path rather than separate synchronous and asynchronous codecs.

The implementation uses Zig 0.16's explicit `std.Io` concurrency model, not the language-level `async`/`await` feature. It owns `std.Io.Operation` storage in a `std.Io.Batch` where an OS-backed operation is appropriate, and uses `std.Io.concurrent` plus a `std.Io.Future` for CPU work such as decompression. The service waits with `Batch.awaitConcurrent` or `Future.await`; callers receive StreamIO request handles, never standard-library storage whose lifetime would be unsafe to expose. A request reaches exactly one terminal state—completed, failed, or canceled—and that state remains queryable even where `std.Io.Batch` omits successfully canceled operations from its completion iteration.

Each request has a result/error, cancellation state, and an unambiguous buffer-ownership contract: the caller owns a supplied buffer until its terminal state is observed; internally allocated result buffers are released only by the Zig-side completion owner. A request must be awaited or canceled before its stream, archive handle, or caller buffer can be destroyed.

Operations against one stream or archive handle are serialized and complete in submission order. Independent handles may run concurrently. Metadata-changing operations (seek, directory enumeration, save/load records, and option XML writes) start on the serialized synchronous control lane; they may move to asynchronous execution only after fixtures demonstrate that observable legacy ordering is unchanged. Cancellation is best-effort: a request that has not begun reaches the canceled terminal state; an in-flight request may complete normally or with an OS error, but never writes to or references a released buffer after its caller has observed terminal state.
