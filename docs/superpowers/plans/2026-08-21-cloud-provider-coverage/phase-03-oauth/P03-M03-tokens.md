...
- [ ] Persist `IsPassword` per field alongside the existing secret flag from P01-M02, so the read-back filter works with no catalogue cached — the same reason that flag is stored rather than re-derived.
