...
- [ ] Model every field the **backend** catalogue provides, which is the union across all backends and exactly these eighteen: `Name`, `Type`, `Help`, `Required`, `Advanced`, `Examples`, `IsPassword`, `Sensitive`, `Hide`, `Default`, `DefaultStr`, `Exclusive`, `FieldName`, `NoPrefix`, `ShortOpt`, `Provider`, `Value`, `ValueStr`.
- [ ] Take that union across backends rather than from one option — `Provider` is absent from s3's first option yet present on 35 options overall, which is precisely how it gets missed.
- [ ] Note that `Groups`, which appears in rclone's general option-block documentation, is **not** present on any backend option record in v1.75.0. It belongs to the global options surface. Tolerate it if a later version adds it, per the unknown-field rule below, but do not model a field the data does not have.
- [ ] Model `Provider` on **examples** too. It appears on 664 of them: s3's `region` examples are provider-conditioned, and rendering the AWS ones for Wasabi would offer regions that do not exist there.
- [ ] **Implement `matchProvider(expression, selected)` here, in `catalogue.zig`, as the single owner.** Three later packets need it — the vendor-change cleanup in P01-M03, the form filter in P02-M01, and required-field validation in P02-M04 — and three implementations of one matching rule will drift. Everything downstream consumes this one.
- [ ] Match rclone exactly: return true when the expression is empty **or when the selected provider is empty**; otherwise strip a leading `!` as negation and test membership in the comma-separated list. Both empty cases matching is what makes a backend with no vendor chosen show all conditional fields rather than none.
- [ ] Test the operators, not the instances: empty expression, empty selection, single match, comma-list membership and non-membership, negation both ways. v1.75.0 has 72 distinct expressions with one negation, but the next version's mix will differ.
