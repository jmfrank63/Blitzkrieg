...
- [ ] **A vendor change is not a backend change, and needs its own rule.** Switching S3 from AWS to Wasabi leaves the backend as `s3`, so backend-scoped preservation keeps every stored option — including AWS-only ones like a `region` the new vendor never declares. Those are then still sent to rclone on the next save.
- [ ] When the `provider` value changes, drop stored options whose `Provider` expression no longer matches, and keep the rest. Add an AWS-to-Wasabi persistence test asserting the AWS-only value is gone and a shared value survived.
