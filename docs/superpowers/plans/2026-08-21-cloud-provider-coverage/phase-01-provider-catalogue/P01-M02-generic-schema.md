...
- [ ] Emit a **flat `Name` → value map** in `parameters`. Backend configuration is keyed by `Option.Name`; `FieldName` belongs to rclone's global RC option-struct JSON, not to `config/create`. Verified against v1.75.0: no backend option has a `FieldName` that differs from its `Name`, no option name contains a dot, and a flat `Name`-keyed create round-trips through `config/get` unchanged. Nesting on a dot would be inventing a case rclone does not have.
