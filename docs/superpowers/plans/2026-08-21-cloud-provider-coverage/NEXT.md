...
sync — is deferred to phase 04 acceptance, which needs a real account).
Every checkpoint is in its phase
...
| coverage review follow-up | `0e8b7667e`..`60848f499` | config flow bound to its handle; saves win over read-back, in-request window included |
...
Suites, all green at `60848f499` (creds 19, catalogue 20, form 9, worker 15,
...
zig build test-cloudsync-worker    -Dtarget=aarch64-macos -Dtest-mode=run   # 15
