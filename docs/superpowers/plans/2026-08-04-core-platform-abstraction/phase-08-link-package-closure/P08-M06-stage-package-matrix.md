...
- [x] Define exact target manifests for Game, PlatformRuntime, modules, SDL3, renderer, and target shader/config requirements. `verify_runtime.zig` now walks staged roots and rejects missing, duplicate, foreign, unsafe, cache/temp, and user-write entries; Windows staging also includes canonical `LICENSE.md` and `README.md`.
