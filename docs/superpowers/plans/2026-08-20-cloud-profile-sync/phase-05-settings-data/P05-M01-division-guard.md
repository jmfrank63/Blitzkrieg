# P05-M01 — division constants and bounds guard
...
- [ ] Write the failing case first: force a synthetic seventh division and observe the current behaviour before changing anything.
- [ ] Correct `_E_BUTTON_CHANGE_DIVISION_END` from `10009` to `10012` and `_E_LIST_END` from `1002` to `1005`, matching the six button and six list elements actually present in `Data/UI/OptionsSettings.xml`. Both constants are currently unused, so today they are simply documentation that is wrong.
