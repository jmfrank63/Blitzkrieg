...
- [ ] **Make `save` invalidate the discovery cache.** `rclone_path` lives in this file, and `P00-M04` caches discovery so `available` and `discovery_status` agree; without invalidation a player who points the game at a working rclone still sees it reported as missing until the next launch. `creds_save` calls `refresh_discovery` after a successful write, so both exports reflect the new path on the very next call.
- [ ] Refresh through the P00-M04 locking contract rather than reaching into the cache: this call arrives on the UI thread while the sync worker may be reading, and the discipline that no caller holds a pointer into the cache is what makes replacing it safe.
- [ ] Add the end-to-end test: with no rclone on `PATH`, `available` is false and `discovery_status.reason` is `not_found`; save credentials carrying a valid `rclone_path`; without restarting anything, assert `available` is now true and `discovery_status.path` is the override. That test is the contract the dialog's "re-run discovery when the path changes" promise rests on.
