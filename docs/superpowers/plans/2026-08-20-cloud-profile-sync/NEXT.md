...
Resume at `phase-02-sync-engine/P02-M01-pairing.md`.

**Phases 00 and 01 are complete.** Phase 00's gate is met
...
**Done so far:** all of phases 00 and 01. Five build steps green natively on
Windows — 72 CloudSync tests — with `test-streamio` unaffected. On the
...
**Next:** `phase-02-sync-engine/P02-M01-pairing.md` — the first packet that
drives a live bisync: mkdir before pairing, sentinel seeding via
`remote_has_sentinel`, resync with the trash assertions in both directions.
Needs the per-machine rclone binary for its live cases.
