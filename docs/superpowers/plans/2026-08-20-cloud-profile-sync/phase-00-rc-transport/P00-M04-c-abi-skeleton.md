...
- [ ] **Stamp each refresh with a generation, or probing outside the lock makes the cache stale rather than unsafe.** Two refreshes overlapping — a credentials save while a retry is already in flight — can finish in either order, and the one that started first can land last and reinstate the path the player just replaced. The mutex prevents the use-after-free; it does nothing about ordering.
- [ ] Take a ticket under the lock (`my_gen = ++next_gen`), release, probe, then re-acquire and publish **only if `my_gen == next_gen`** — that is, only if no refresh has started since this one. A result that has been superseded is discarded, not written.
- [ ] The weaker test `my_gen > published_gen` is not enough, and fails in the case that matters: with generation 1 and 2 both in flight and nothing published yet, generation 1 satisfies `1 > 0` and installs its result while 2 is still probing. Since generation 2 is typically the credentials save that just changed the path, the cache would serve the superseded path to the worker for exactly as long as the newer probe takes.
- [ ] A superseded refresh reports success to its own caller; it simply does not publish. And if the newest refresh fails, the cache keeps its previous value rather than adopting an older probe's — stale-but-known beats confidently wrong.
