...
- [ ] Take a ticket under the lock (`my_gen = ++next_gen`), release, probe, then re-acquire and publish **only if `my_gen > published_gen`**, setting `published_gen = my_gen`. A result that lost the race is discarded, not written.
- [ ] Test it with two refreshes forced to complete in reverse order — a stubbed probe with controllable delay — asserting the cache holds the later-started result. Serialising refresh writers behind their own lock is an acceptable alternative, but only if the test still demonstrates the ordering it claims.
