...
- [ ] Prove the property the approach rests on: rclone does **not** resolve a symlinked root, so the short name survives into the session name. Measured on macOS — a 199-byte directory reached through an 8-byte link produced a 21-byte session name — and **unproven for Windows junctions**. Prove it here or stop and report.
