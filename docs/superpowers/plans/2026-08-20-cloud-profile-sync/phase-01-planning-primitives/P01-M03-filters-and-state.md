...
- [ ] **Fix the remote layout so nothing that is not a profile lives under Path2.** Path2 is `<remote>/profiles/<name>/`; the trash and the config backups are siblings of `profiles/`, not children of it:
  ...
  Anything placed beneath Path2 is synced back down by definition. The `config-backups/**` and trash filter entries above are the belt-and-braces guard for a path that is *already* outside the prefix, not the primary mechanism — a filter that is the only thing standing between backup history and every machine is one typo from a surprise.
