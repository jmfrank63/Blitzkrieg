...
- [ ] **Emit two trash directories, on their own filesystems, unique per run.** rclone requires each backup directory to sit on its own side's filesystem — a local path passed as `backupDir2` against a remote Path2 fails the run with `parameter to --backup-dir has to be on the same remote as destination` (verified). Emit `backupDir1` as `<local trash>/<run_id>` and `backupDir2` as `<remote>/trash/<profile>/<run_id>`, where `run_id` is a UTC timestamp plus a random nonce.
- [ ] **The per-run directory is not tidiness, it is the safety net.** rclone overwrites an existing backup at the same resulting path: deleting `saves/m2.sav`, recreating it, and deleting it again left only the second version in the trash and destroyed the first (verified). Save filenames are reused constantly — `quick.sav` and the autosaves are the same name every time — so a shared trash root loses recovery copies during ordinary play, not in some edge case. Add `repeated_overwrite_keeps_every_version` to the test.
- [ ] A per-run `_config.Suffix` is an equivalent fix and also verified working, but only if the suffix itself is unique per run; a constant suffix collapses exactly like a shared root. Prefer the per-run directory, which makes pruning a directory-level operation.
