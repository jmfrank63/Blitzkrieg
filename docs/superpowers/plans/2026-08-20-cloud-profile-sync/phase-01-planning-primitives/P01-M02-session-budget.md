...
- [ ] Write the failing test from the measured cases: a 249-byte session name must be rejected and `tmp_bkp..tmp_bkremote` accepted.
...
- [ ] Implement `fsPath(allocator, path, kind) ![]u8` mirroring `bilib.FsPath`: a local path contributes its raw absolute form with a trailing separator; a named remote contributes only `name + ":" + root`. Only the local branch is expensive, which is why Path2 must always be a named remote.
...
- [ ] Implement `checkSessionBudget(p1, p2) !void` returning `error.SessionNameTooLong` carrying the projected length, so the caller reports a number rather than "sync failed".
