...
- [ ] State that the restore is staged and takes effect at the next launch. The game rewrites `config.cfg` from memory at shutdown and from eight other `SerializeConfig` call sites, which is exactly why the restore is staged rather than applied live — see P04-M03. Settings the player changes before restarting are not lost: the merge runs at startup against the file as it then stands.
