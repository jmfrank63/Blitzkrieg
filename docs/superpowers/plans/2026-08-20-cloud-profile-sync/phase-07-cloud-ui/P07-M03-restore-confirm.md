...
- [ ] Offer undo from what `restore_undo_available` reports, which is now three answers rather than a bool, and **name the action for the state it is in**: "Cancel pending restore" when a stage is waiting to apply, "Undo applied restore" when a previous one has already been installed. The two do visibly different things — one discards a restore that has not happened, the other reverses one that has — and a single "Undo" label would leave the player guessing which.
- [ ] Disable the control while the slot reports busy, i.e. while a restore download is in flight. Leaving it live there is what allows an undo the finishing download would silently overwrite.
- [ ] State that the restore is staged and takes effect at the next launch. The game rewrites `config.cfg` from memory at shutdown and from eight other `SerializeConfig` call sites, which is exactly why the restore is staged rather than applied live — see P04-M03. Be precise about what survives: the merge runs at startup against `config.cfg` as it then stands, so **local `GFX.*` changes made before restarting are kept** — that is the whole point of merge mode. Every other key comes from the backup by design, so a sound or gameplay setting changed after requesting the restore *is* replaced by the backup's value. Say that plainly rather than implying nothing is lost.
