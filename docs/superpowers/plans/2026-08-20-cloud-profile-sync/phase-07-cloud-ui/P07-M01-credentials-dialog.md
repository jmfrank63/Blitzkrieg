...
- [ ] **Add the rclone binary path field.** The feature is unavailable without the binary, bundling and auto-download are both out of scope, and `Credentials.rclone_path` already exists to hold it — so this field is the only way a player with rclone installed somewhere unusual can turn the feature on at all. Show the discovered path when one was found, the typed rejection reason when it was not (`.not_found`, `.too_old`, `.not_executable`), and let the player type or browse to an override.
- [ ] Re-run discovery when the path changes and report the result immediately, rather than leaving the player to infer it from a failed sync.
