...
- [ ] If a restore is still staged and unapplied, undo cancels it rather than reinstating anything — there is nothing to reinstate yet, and rewriting the config would be wrong. Cancel in the same order the apply tears down with: **remove `ACTIVE` first, then garbage-collect the stage directories.** Deleting the directory first would recreate the dangling-`ACTIVE` state that P04-M03 has to treat as a hard error.
- [ ] `restore_undo_available` reports on two distinct states, so it must test for both: `ACTIVE` present means a staged restore can be cancelled, and `LATEST_UNDO` present means an applied restore can be reinstated. There is no "pending file" to look for — that name predates the staged-directory protocol.
- [ ] It must also report **busy** while a restore download holds the operation slot from P04-M03. Reporting available in that window is what lets a stale `LATEST_UNDO` invite an undo that the finishing download then overwrites, so availability has to distinguish three answers — cancellable, reinstatable, busy — rather than two.
- [ ] Export `bk_cloudsync_restore_undo() i32` and `bk_cloudsync_restore_undo_available() u32`, with undo branching on which state it finds and both taking the shared operation slot before touching `ACTIVE`. Own the whole export path.
