...
**Evidence:** Unit tests show a byte-identical undo after both a merge restore and a full restore across a simulated restart, an undo that recovers a change written *after* staging but before application, and the staged-but-unapplied case where undo discards the stage.
