...
- [ ] Place `config-backups/` outside the synced prefix and confirm `config.cfg` remains excluded from the sync filter set. It is backed up, never synced — machine-specific `GFX.Mode`, `GFX.Monitor`, and `GFX.FullScreen` are exactly what this split protects.
- [ ] Commit checkpoint: `cloudsync: snapshot config.cfg per host`.

**Evidence:** Integration output shows a snapshot at the expected per-host path, outside the synced prefix, with `config.cfg` still absent from the sync set.
