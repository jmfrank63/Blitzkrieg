...
- [ ] **Apply any staged config restore in this same early window**, before `OptionSystem::Init()` reads `config.cfg`. This is the only point where the option system has not yet loaded and no `SerializeConfig` can have run, which is why P04-M03 stages rather than writes.
- [ ] Apply it with a single `NCloudSync::ApplyPendingRestore( szProfile )` call. **Do not reimplement the merge in C++**: `mergeConfig` lives in Zig (P04-M03) and a second implementation against the same config format would drift from the first. This packet decides *when* to apply, not *how*.
- [ ] Call it unconditionally in that window and treat the "nothing staged" return as the ordinary case, rather than probing for the stage directory in C++.
- [ ] Apply the stage even when cloud sync is disabled or rclone is missing. The restore was already requested and downloaded; refusing to finish it because the feature is now off would strand the player's config in limbo. The apply export is deliberately local-only so this works.
