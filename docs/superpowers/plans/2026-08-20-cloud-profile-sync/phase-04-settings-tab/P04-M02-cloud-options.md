# P04-M02 — Cloud option declarations
...
- [ ] Declare exactly five options, all `EditorType 3` (`EOET_CLICK_SWITCHES`, droplist of string values): `Cloud.Enabled`, `Cloud.Provider` (`Off`/`S3`/`WebDAV`), `Cloud.Sync.OnStartup`, `Cloud.Sync.OnExit`, `Cloud.Config.Backup`.
- [ ] **Declare no string-valued free-text option here.** `COptionSystem::Set` truncates any string over 12 characters to 8 unless the option's action is `SetVideoMode` (`OptionSystemInternal.cpp:186-192`), which would silently destroy an endpoint, key, or secret.
...
**Evidence:** Headless capture shows five tabs — GFX, GamePlay, Multiplayer, Sound, Cloud — with the Cloud list populated and the in-mission options screen unchanged.
