...
function Objective3()
	if ( GetNUnitsInScriptGroup(15, 1) <= 0) then
		SetIGlobalVar("temp.template01.objective.3", 1);
...
function ToWin()
	if ( (GetIGlobalVar("temp.template01.objective.0", 0) * GetIGlobalVar("temp.template01.objective.1", 0) * GetIGlobalVar("temp.template01.objective.2", 0) *
	  GetIGlobalVar("temp.template01.objective.3", 0)) == 1) then
		Win(0);
		Suicide();
	end;
end;
