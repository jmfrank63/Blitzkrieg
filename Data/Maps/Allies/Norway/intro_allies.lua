...
local ACTION_STOP = 9;
	if ( GetNUnitsInScriptGroup(1, 1) <= 0) then
		SetIGlobalVar( "temp.intro_allies.objective.0", 1);
...
local FORMATION_AGGRESSIVE = 3;
	...
	if ( GetSquadInfo( 1001) == FORMATION_AGGRESSIVE) then
		SetIGlobalVar( "temp.intro_allies.objective.1", 1);
...
function Objective2()
	if ( GetNUnitsInScriptGroup(2, 1) <= 0) then
		SetIGlobalVar( "temp.intro_allies.objective.2", 1);
...
local STATE_TOWED = 24;
	...
	if ( GetUnitState( 1010) ~= STATE_TOWED) then
		SetIGlobalVar( "temp.intro_allies.objective.3", 1);
...
function Objective4()
	if (GetIGlobalVar("Mission.Current.ObjectiveShown", 0) == 0) then
		SetIGlobalVar( "temp.intro_allies.objective.4", 1);
...
local ACTION_STOP = 9;
	...
	if ( GetNUnitsInScriptGroup(3, 1) < 3) then
		SetIGlobalVar("temp.intro_allies.objective.5", 1);
...
function Objective6()
	if (GetIGlobalVar("Mission.Current.ObjectiveShown", 0) == 0) then
		SetIGlobalVar( "temp.intro_allies.objective.6", 1);
...
function Objective7()
	if ( (GetNUnitsInScriptGroup(4, 1) <= 0) and ( GetNUnitsInScriptGroup(5, 0) > 0)) then
		SetIGlobalVar("temp.intro_allies.objective.7", 1);
...
local STATE_REST_ON_BOARD = 3;
	if ( (GetNScriptUnitsInArea(1002, "Station") > 0) and ( GetUnitState( 1002) ~= STATE_REST_ON_BOARD)) then
		SetIGlobalVar("temp.intro_allies.objective.8", 1);
...
function Objective9()
	if ( GetNUnitsInScriptGroup(6, 1) <= 0) then
		SetIGlobalVar("temp.intro_allies.objective.9", 1);
...
function Objective10()
	if ( GetAviationState( 0) > -1) then
		SetIGlobalVar("temp.intro_allies.objective.10", 1);
...
function ToWin()
local num = GetIGlobalVar("temp.intro_allies.objective.0", 0) * GetIGlobalVar("temp.intro_allies.objective.1", 0) * GetIGlobalVar("temp.intro_allies.objective.2", 0) *
	GetIGlobalVar("temp.intro_allies.objective.3", 0) * GetIGlobalVar("temp.intro_allies.objective.5", 0) * GetIGlobalVar("temp.intro_allies.objective.7", 0) *
	GetIGlobalVar("temp.intro_allies.objective.8", 0) * GetIGlobalVar("temp.intro_allies.objective.9", 0) * GetIGlobalVar("temp.intro_allies.objective.10", 0);
	if ( num == 1) then
		ObjectiveChanged(12, 1);
		SetIGlobalVar( "temp.intro_allies.objective.12", 1);
		SetIGlobalVar( "temp.intro_allies.objective.11", 1);
...
function RevealObjective0()
	if ( GetIGlobalVar("temp.intro_allies.objective.0", 0) == 0) then
...
function RevealObjective1()
	if ( GetIGlobalVar( "temp.intro_allies.objective.1", 0) == 0) then
...
function RevealObjective2()
	if ( GetIGlobalVar("temp.intro_allies.objective.2", 0) == 0) then
...
function RevealObjective3()
	if ( GetIGlobalVar("temp.intro_allies.objective.3", 0) == 0) then
...
function RevealObjective4()
	if ( GetIGlobalVar("temp.intro_allies.objective.4", 0) == 0) then
...
function RevealObjective5()
	if ( GetIGlobalVar("temp.intro_allies.objective.5", 0) == 0) then
...
function RevealObjective6()
	if ( GetIGlobalVar("temp.intro_allies.objective.6", 0) == 0) then
...
function RevealObjective7()
	if ( GetIGlobalVar("temp.intro_allies.objective.7", 0) == 0) then
...
function RevealObjective8()
	if ( GetIGlobalVar("temp.intro_allies.objective.8", 0) == 0) then
...
function RevealObjective9()
	if ( GetIGlobalVar("temp.intro_allies.objective.9", 0) == 0) then
...
function RevealObjective10()
	if ( GetIGlobalVar("temp.intro_allies.objective.10", 0) == 0) then
		SwitchWeather( 0);
...
local STATE_SWARM = 11;
	if (( GetUnitState( 999) == STATE_SWARM) or ( GetIGlobalVar( "temp.intro_allies.objective.0", 0) == 1)) then
		Suicide();
	else
...
local FORMATION_AGGRESSIVE = 3;
	if (( GetSquadInfo( 1001) == FORMATION_AGGRESSIVE) or ( GetIGlobalVar( "temp.intro_allies.objective.1", 0) == 1)) then
		Suicide();
	else
...
local STATE_SUPPRESSIVE_FIRE = 34;
	if (( GetUnitState( 1010) == STATE_SUPPRESSIVE_FIRE) or ( GetIGlobalVar( "temp.intro_allies.objective.5", 0) == 1)) then
		Suicide();
	else
